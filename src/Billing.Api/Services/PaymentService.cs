...
public sealed class PaymentService(
    ...
    IIdempotencyLock idempotencyLock) : IPaymentService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var lease = await idempotencyLock.AcquireAsync(
    ...
    private async Task<PaymentResult> ExecutePaymentAsync(
        ...
        CancellationToken cancellationToken)
    {
        using var invoiceLease = await idempotencyLock.AcquireAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return PaymentResult.Failed(
                record.StatusCode ?? StatusCodes.Status500InternalServerError,
    ...
    private void MarkPaymentFailed(Payment payment, IdempotencyRecord record)
    {
        ...
        record.StatusCode = StatusCodes.Status502BadGateway;
        record.ResponsePaymentId = payment.Id;
